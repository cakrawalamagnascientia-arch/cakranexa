...
import { allowedProviders, DEFAULT_PAYMENT_ROUTING, MANAGED_ROUTING_TYPES, type ManagedRoutingType, type RoutingEntry, type RoutingMethod, type RoutingProvider } from '../data/paymentRouting';
...
/**
 * Metode pembayaran per jenis transaksi (tabel payment_routing) dan pengaturan transfer manual buku cetak (kode unik,
 * batas waktu). Bawaan: buku cetak & keanggotaan hanya transfer bank ke rekening PT; pembelian satuan produk digital
 * off (fase 6); metode lain off sampai diaktifkan di sini.
 */
const TYPE_INFO: Record<ManagedRoutingType, { title: string; note: string }> = {
  print: {
    title: 'Checkout buku cetak',
    note: 'Bila hanya satu metode aktif, pembeli tidak melihat pilihan metode dan langsung mendapat instruksi transfer.'
  },
  membership: {
    title: 'Langganan Blue/Silver/Gold/Platinum',
    note: 'Transfer bank dikonfirmasi Finance. Metode Midtrans hanya berlaku bila server Midtrans aktif.'
  },
  digital: {
    title: 'Pembelian satuan e-book/audiobook',
    note: 'Fase 6 tidak menjual satuan: biarkan semua off. Mengaktifkan Midtrans di sini membuka kembali tombol beli satuan.'
  }
};
const TYPE_ORDER: ManagedRoutingType[] = ['print', 'membership', 'digital'];
...
export const PaymentRoutingPanel: React.FC = () => {
  ...
  const activeOf = (type: ManagedRoutingType) => routing.filter((r) => r.transactionType === type && (r.provider === 'manual' || r.provider === 'midtrans')).length;
  const active = activeOf('print');
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-xs sm:p-6" id="payment-routing-panel">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-3">
        <div>
          <h3 className="flex items-center gap-2 text-sm font-bold text-slate-900"><CreditCard className="h-4 w-4 text-[#D4AF37]" />Metode Pembayaran (payment_routing)</h3>
          <p className="mt-0.5 text-xs text-slate-500">
            Sumber tunggal metode pembayaran per jenis transaksi.
            {active === 1 ? ' Buku cetak: satu metode aktif.' : ` Buku cetak: ${active} metode aktif.`}
          </p>
        </div>
      ...
      {TYPE_ORDER.filter((type) => (MANAGED_ROUTING_TYPES as readonly string[]).includes(type)).map((type) => (
        <div key={type} className="space-y-2" data-routing-type={type}>
          <div>
            <h4 className="text-xs font-bold text-slate-900">{TYPE_INFO[type].title} <span className="font-normal text-slate-500">· {activeOf(type)} metode aktif</span></h4>
            <p className="text-[11px] text-slate-500">{TYPE_INFO[type].note}</p>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {routing.filter((r) => r.transactionType === type).map((entry) => (
              <label key={`${type}-${entry.method}`} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 p-2.5 text-xs">
