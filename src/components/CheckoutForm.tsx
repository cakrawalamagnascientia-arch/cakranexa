...
export const CheckoutForm: React.FC<CheckoutFormProps> = ({
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    const randomVa = generateSimulatedVa(customer.phone);
    const autoTrackingNumber = generateCakraNexaTrackingNumber();
    setGeneratedVa(randomVa);
    ...
    // 3) Midtrans asli (Snap popup) jika token tersedia; jika tidak -> layar simulasi sandbox
    if (newOrder.snapToken && newOrder.snapToken !== 'SIMULATION') {
      const opened = await openSnapPayment(newOrder.snapToken, {
        onSuccess: () => finalizeOrder(newOrder, 'paid'),
    ...
    setIsSubmitting(false);
    setCheckoutStep('midtrans_simulation');
  };

  /** Menyelesaikan pesanan setelah callback Midtrans (status final tetap ditentukan webhook di server). */
  ...
  // Simulation handler (hanya aktif jika MIDTRANS_SERVER_KEY belum dikonfigurasi di server)
  const handleCompleteMidtransPayment = async () => {
    if (!createdOrder) return;
    setIsSubmitting(true);
    await finalizeOrder(createdOrder, 'paid');
  };
            ...
            <div className="p-6 space-y-6">
              ...
              {createdOrder.paymentMethod === 'qris' || ['gopay', 'ovo', 'dana', 'shopeepay'].includes(createdOrder.paymentMethod) ? (
                ...
                <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 text-center space-y-3">
                  ...
                  <div className="font-mono text-2xl font-bold tracking-widest text-[#0F172A]">
                    {generatedVa || '8801239849201948'}
                  </div>
